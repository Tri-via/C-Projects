...
//protected:
    ...
    Node<Key, Value>* del(const Key& key);
    bool isBal(Node<Key, Value>* node) const;
...
BinarySearchTree<Key, Value>::iterator::operator++()
{
     current_ = getSuccessor(current_);
    return *this;
}
    ...
    while(node != NULL){
        ...
        if(node->getKey() < parent->getKey()){
            if(parent->getLeft() != NULL){
                ...
                Node<Key, Value>* newRoot = new Node<Key, Value>(keyValuePair.first, keyValuePair.second, parent);
        ...
        //if the value inserted is equal
        else {
...
template<typename Key, typename Value>
Node<Key, Value>* BinarySearchTree<Key, Value>::del(const Key& key){
...
template<typename Key, typename Value>
bool BinarySearchTree<Key, Value>::isBal(Node<Key,Value>* node) const
{
    if(node==NULL && std::abs(getSubtreeHeight(node->getLeft()) - getSubtreeHeight(node->getRight())) <= 1 && isBal(node->getLeft()) && isBal(node->getRight()))
        return true;
    return false;
}
